...
class RoomRegistryBackend:
    ...
    async def _create_room(
        ...
        async with runtime.lifecycle_lock:
            ...
            await actor.publish_per_seat(
                lambda seat: _state_message(core.projection_for_seat(seat))
            )
    ...
    async def _join_room(
        ...
        async with runtime.lifecycle_lock:
            ...
            if runtime.core.state_version != before:
                await runtime.actor.publish_per_seat(
                    lambda seat: _state_message(
                        runtime.core.projection_for_seat(seat)
                    )
                )
    ...
    async def _submit_room_command(
        ...
        async with runtime.lifecycle_lock:
            ...
            left = bool(response_dict.get("ok")) and not _token_is_active(
                runtime.core,
                token,
            )
...
def _token_is_active(core: RoomCore, token: str) -> bool:
    try:
        core.seat_for_token(token)
    except RoomError:
        return False
    return True
